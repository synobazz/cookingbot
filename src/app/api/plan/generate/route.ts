...
export async function POST(req: NextRequest) {
  ...
  if (recipes.length === 0) return NextResponse.json({ error: "Keine Rezepte im Cache. Bitte zuerst Paprika synchronisieren." }, { status: 400 });
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      { role: "system", content: "Du bist eine smarte, familienfreundliche Kochhilfe. Erzeuge ausschließlich valides JSON nach dem verlangten Schema. Plane Abendessen, vermeide direkte Wiederholungen und nutze Paprika-Rezepte oder plausible Remixe/Beilagen daraus." },
